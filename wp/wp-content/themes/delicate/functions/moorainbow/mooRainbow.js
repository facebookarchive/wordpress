...
var MooRainbow = new Class({
	...
	hsv: function(rgb) {
		var r = rgb[0], g = rgb[1], b = rgb[2], H, S, V;
	...
	update: function(input, color) {
		input.setStyle('background-color', color.hex).value = color.hex;
						
		var hsv = this.hsv(color.rgb);
		if (hsv[2] < 127 || hsv[0] > 0.5) input.setStyle('color', '#fff');
		else input.setStyle('color', '#000');
