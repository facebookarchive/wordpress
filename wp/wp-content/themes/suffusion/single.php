			...
			if (is_array($suffusion_post_types)) {
				foreach ($suffusion_post_types as $suffusion_post_type) {
					if ($suffusion_post_type['style_inherit'] != 'custom') {
						$custom_class = array($suffusion_post_type['style_inherit']);
					}
				}
			}
			if ($custom_class == "") {
				$custom_class = array("post");
			}
			if (isset($suffusion_cpt_layouts[$post->post_type]) && isset($suffusion_cpt_layouts[$post->post_type]['byline'])) {
				$custom_class[] = $suffusion_cpt_layouts[$post->post_type]['byline'];
			}
		}
		$custom_class[] = 'fix';
?>
	<article <?php post_class($custom_class);?> id="post-<?php the_ID(); ?>">
<?php
